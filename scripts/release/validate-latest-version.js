...
function validateLatestVersion(version) {
  ...
  if (newPreRelease !== null) {
    throw new Error(
      `Pre-release version ${version} cannot be the latest version`
    );
  }

  if (newMajor < major) {
    throw new Error(
      `New major version ${newMajor} is less than latest major version ${major}`
    );
  }

  // For enriched markdown, we allow major version bumps
  const isValid =
