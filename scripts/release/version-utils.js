...
function getStableBranchVersion() {
  ...
  try {
    ...
    throw new Error(
      `Failed to parse stable version from branch: ${currentBranch}`
    );
  }
}

function getLatestVersion() {
  const latestVersion = getPackageVersionByTag(
...
function getNextStableVersion() {
  ...
  while (true) {
    ...
    try {
      ...
      getPackageVersionByTag('react-native-enriched-markdown', version);
      nextPatch++;
    } catch (error) {
...
function getNextPreReleaseVersion(releaseType, version) {
  ...
  while (true) {
    ...
    try {
      ...
      getPackageVersionByTag('react-native-enriched-markdown', targetVersion);
      dotIndex++;
    } catch (error) {
