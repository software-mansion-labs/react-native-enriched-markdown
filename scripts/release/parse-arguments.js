...
function parseArguments() {
  ...
  assert(
    version === null || isBeta || isReleaseCandidate || !isNightly,
    'Version should not be provided for nightly releases'
  );
  ...
  if (version != null) {
    ...
    if (!versionRegex.test(version)) {
      throw new Error(
        `Provided version "${version}" is not valid. Expected format: x.y.z`
      );
    }
  }
