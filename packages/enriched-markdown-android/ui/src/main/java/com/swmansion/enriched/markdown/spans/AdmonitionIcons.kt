 ...
 * and must stay byte-identical to the other renderers so the glyph looks the same
 * everywhere:
 *   - Web:     react-native-enriched-markdown/src/web/renderers/admonitionIcons.ts (source of truth)
 *   - iOS:     react-native-enriched-markdown/ios/segments/ENRMAdmonitionIcons.m
 *   - Android: react-native-enriched-markdown/android/.../segments/AdmonitionIcons.kt
 *   - this file
 *
 * note=info, tip=light-bulb, important=report, warning=alert, caution=stop.
 *
 * NOTE: the first three copies are kept in sync by
 * react-native-enriched-markdown/__tests__/admonition-icons-parity.test.ts, which only
 * reads files inside that package. This copy lives outside it and is therefore NOT
 * covered by that guard — when the octicon data changes, update this file by hand.
 */
