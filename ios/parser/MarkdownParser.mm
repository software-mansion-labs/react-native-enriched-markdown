...
{
  if (!node || !context || !context->nodeStack)
    return;
...
static NSString *stringFromMDAttribute(const MD_ATTRIBUTE *attr)
{
  ...
  return [[NSString alloc] initWithBytes:attr->text length:attr->size encoding:NSUTF8StringEncoding];
}
...
static int md4c_text_callback(MD_TEXTTYPE type, const MD_CHAR *text, MD_SIZE size, void *userdata)
{
  ...
  if (size > 0 && (type == MD_TEXT_NORMAL || type == MD_TEXT_CODE)) {
    NSString *textString = [[NSString alloc] initWithBytes:text length:size encoding:NSUTF8StringEncoding];
    if (textString) {
...
- (MarkdownASTNode *)parseMarkdown:(NSString *)markdown
{
  ...
  // Initialize context
  MD4CContext context = {.root = nil, .nodeStack = [[NSMutableArray alloc] init]};

  // Configure MD4C parser with callbacks
  MD_PARSER parser = {.enter_block = md4c_enter_block_callback,
